...
function renderSettingControl(s) {
  ...
  if (SELECT_OPTIONS[key]) {
    ...
    return `<select data-key="${escapeHtml(key)}" onchange="onSettingChange(this)">
  ...
  if (typeof value === "boolean") {
    return `<input type="checkbox" data-key="${escapeHtml(key)}" ${value ? "checked" : ""} onchange="onSettingChange(this)">`;
  }
  ...
  if (typeof value === "number") {
    ...
    return `<input type="number" data-key="${escapeHtml(key)}" value="${value}" step="${step}" ${min !== undefined ? `min="${min}"` : ""} ${max !== undefined ? `max="${max}"` : ""} onchange="onSettingChange(this)">`;
  }

  // Text input
  return `<input type="text" data-key="${escapeHtml(key)}" value="${escapeHtml(String(value))}" onchange="onSettingChange(this)">`;
}
...
async function onSettingChange(el) {
  ...
  // Side effects for model changes
      ...
      .map(
        (e) =>
          `<div class="memory-row">
            ...
            <button class="memory-delete" onclick="deleteMemory('${escapeHtml(e.id)}')" data-tooltip="Delete">&times;</button>
          </div>`
      )
      .join("");
  } catch {
...
async function addMemory() {
  ...
  try {
    const res = await fetch(`${API}/api/memory`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ category: category.value, key: k, value: v }),
    });
